<?php
function livesay_widget() {
	register_widget( 'livesay_instagram_widget' );
}
add_action( 'widgets_init', 'livesay_widget' );
class livesay_instagram_widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'livesay-instagram-feed',
			VTHEME_NAME_P . esc_html__( ': Instagram', 'signy-core' ),
			array(
				'classname' => 'lvsy-widget lvsy-instagram-gallery',
	...
	function widget( $args, $instance ) {
		...
		do_action( 'livesay_before_widget', $instance );
		...
		if ( $username != '' ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				if ( $images_only = apply_filters( 'livesay_images_only', FALSE ) ) {
				...
				$ulclass = apply_filters( 'livesay_list_class', ' instagram-size-' . $size );
				$liclass = apply_filters( 'livesay_item_class', '' );
				$aclass = apply_filters( 'livesay_a_class', '' );
				$imgclass = apply_filters( 'livesay_img_class', '' );
				$template_part = apply_filters( 'livesay_template_part', 'parts/wp-instagram-widget.php' );
...
?>
		...
		$linkclass = apply_filters( 'livesay_link_class', 'clear' );

		do_action( 'livesay_after_widget', $instance );

		echo $args['after_widget'];
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'username' => '', 'size' => 'large','number' => 9, 'target' => '_self' ) );
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'signy-core' ); ?>: <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
	...
	function scrape_instagram( $username ) {
		...
		if ( false === ( $instagram = get_transient( 'instagram-a5-'.sanitize_title_with_dashes( $username ) ) ) ) {
			...
			if ( ! empty( $instagram ) ) {
				...
				set_transient( 'instagram-a5-'.sanitize_title_with_dashes( $username ), $instagram, apply_filters( 'livesay_instagram_cache_time', HOUR_IN_SECONDS*2 ) );
			}
		}
		if ( ! empty( $instagram ) ) {
