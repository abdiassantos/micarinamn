/*
 * All Admin Related Scripts in this Livesay Theme
...
jQuery(document).ready(function($) {
  ...
  $('.vt-reserve-widgt-fields input[type="text"]').keyup(function() {
    $.sanitize = function(input) {
      var output = input.replace('/<script[^>]*?>.*?<\/script>/gi', '').replace('/<[\/\!]*?[^<>]*?>/gi', '').replace('/<style[^>]*?>.*?<\/style>/gi', '').replace('/<![\s\S]*?--[ \t\n\r]*>/gi', '');
        return output;
    };
    var currentText = $(this).val();
    $(".reserve-frm-id").text($.sanitize(currentText));
    // $('.reserve-frm-id').text($(this).val());
  });
